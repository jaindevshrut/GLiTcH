...
### 1️⃣ best.pt (Primary Model)
```
📊 Architecture: Deep neural network with optimized layers
🎯 Specialty: General-purpose deepfake detection
⚡ Performance: ACC 95.18% | EER 4.10%
🔧 Threshold: 0.521
```
...
|-------|-----------|-----|-----|-----|-----|----------|-----------|--------|----------|------|
| **best.pt** | 0.521 | 229 | 5 | 16 | 41 | **95.18%** | 89.15% | 71.79% | 84.86% | 4.10% |
...
#### 🏆 Best Performers:
- **Highest Accuracy**: best.pt (95.18%)
...
│   │                    Feature Extraction                        │   │
│   ├──────────────────┬──────────────────┬───────────────────────┤   │
│   │   Raw Waveform   │   LFCC Features  │   Mel Spectrogram     │   │
...
├──────────────────────────────────────────────────────────────────────┤
│                                                                       │
│   Input Audio ──▶ Resample to 16kHz ──▶ Convert to Mono              │
│                                              │                        │
│                    ┌─────────────────────────┼─────────────────────┐  │
│                    │                         │                     │  │
│                    ▼                         ▼                     ▼  │
│           ┌──────────────┐          ┌──────────────┐      ┌──────────┐│
│           │ Raw Waveform │          │     LFCC     │      │   Mel    ││
│           │   Tensor     │          │  Extraction  │      │Spectogram││
│           │  (1D signal) │          │  (20 coeffs) │      │(128 bins)││
│           └──────────────┘          └──────────────┘      └──────────┘│
│                                                                       │
└──────────────────────────────────────────────────────────────────────┘
```
